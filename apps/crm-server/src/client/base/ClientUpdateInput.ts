/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { ClientInterfaceUpdateManyWithoutClientsInput } from "./ClientInterfaceUpdateManyWithoutClientsInput";
import { Type } from "class-transformer";
import { ContactUpdateManyWithoutClientsInput } from "./ContactUpdateManyWithoutClientsInput";
import { RdpDetailUpdateManyWithoutClientsInput } from "./RdpDetailUpdateManyWithoutClientsInput";

@InputType()
class ClientUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: () => ClientInterfaceUpdateManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => ClientInterfaceUpdateManyWithoutClientsInput)
  @IsOptional()
  @Field(() => ClientInterfaceUpdateManyWithoutClientsInput, {
    nullable: true,
  })
  clientInterfaces?: ClientInterfaceUpdateManyWithoutClientsInput;

  @ApiProperty({
    required: false,
    type: () => ContactUpdateManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => ContactUpdateManyWithoutClientsInput)
  @IsOptional()
  @Field(() => ContactUpdateManyWithoutClientsInput, {
    nullable: true,
  })
  contacts?: ContactUpdateManyWithoutClientsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  flexAccountNumber?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  phoneNumber?: string | null;

  @ApiProperty({
    required: false,
    type: () => RdpDetailUpdateManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => RdpDetailUpdateManyWithoutClientsInput)
  @IsOptional()
  @Field(() => RdpDetailUpdateManyWithoutClientsInput, {
    nullable: true,
  })
  rdpDetails?: RdpDetailUpdateManyWithoutClientsInput;
}

export { ClientUpdateInput as ClientUpdateInput };
